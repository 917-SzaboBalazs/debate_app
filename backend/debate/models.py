...
class DebateManager(models.Manager):
    def create_debate(self, type, entry_code, **other_fields):
        other_fields.setdefault('no_judges', 3)
        other_fields.setdefault('has_chair', False)

        if type == "british":
            return self.create_british_parliamentary_debate(type, entry_code, **other_fields)
        return None

    def create_british_parliamentary_debate(self, type, entry_code, **other_fields):
        other_fields['team_size'] = 4

        debate = self.model(type=type, entry_code=entry_code, **other_fields)
